...
if ((rVersion) and (tonumber(rVersion) ~= nil)) then
	if (tonumber(sVersion) < tonumber(rVersion)) then
		...
		DownloadFile('https://raw.githubusercontent.com/pvpsuite/BoL/master/Scripts/p_Spotify.lua?no-cache=' .. math.random(1, 25000), (SCRIPT_PATH.. GetCurrentEnv().FILE_NAME), function()
			print('<font color="#FF1493"><b>[p_Spotify]</b> </font><font color="#00FF00">Script has been updated, please reload!</font>');
		end);
		return;
	end;
else
...
function OnDraw()
	if ((theMenu.showPlayer) and (readyToShow)) then
		...
		DrawText(theSong, 12, (theMenu.drawW + (backgroundW / 2) - (theSongA.x / 2)), theMenu.drawH + 5, RGBA(255, 255, 255, theMenu.playerOpacity));
		DrawText(theArtist, 14, (theMenu.drawW + (backgroundW / 2) - (theArtistA.x / 2)), theMenu.drawH + 20, RGBA(255, 255, 255, theMenu.playerOpacity));
	end;
end;
