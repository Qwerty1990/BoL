...
if ((rVersion) and (tonumber(rVersion) ~= nil)) then
	if (tonumber(sVersion) < tonumber(rVersion)) then
		...
		DownloadFile('https://raw.githubusercontent.com/pvpsuite/BoL/master/Scripts/p_modelChanger.lua?no-cache=' .. math.random(1, 25000), (SCRIPT_PATH.. GetCurrentEnv().FILE_NAME), function()
			print('<font color="#FF1493"><b>[p_modelChanger]</b> </font><font color="#00FF00">Script successfully updated, please double-press F9 to reload!</font>');
		end);
		return;
	end;
else
...
function OnUnload()
	if (theMenu.changeModel) then
		if (lastActiveOption ~= 0) then
			SendModelPacket(myHero.charName);
		end;
	end;
end;
...
function OnRecvPacket(sPacket)
	if (sPacket.header == 0xB6) then
		if (theMenu.changeModel) then
			local activeOption, menuOption = getActiveMenuSelection();
			DelayAction(function() SendModelPacket(getModelObject(activeOption, menuOption)); end, 0.01);
		end;
	end;
end;
